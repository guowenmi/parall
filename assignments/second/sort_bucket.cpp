...
float *bucket = calloc(bucketCount * nrecv, sizeof(float));
int *itemCount = calloc(bucketCount, sizeof(int));
...
MPI_Alltoall(itemCount,
1, MPI_INT, recvCount, 1, MPI_INT, MPI_COMM_WORLD);
...
qsort(big_bucket, totalCount,
sizeof(float), floatComparator);
double sorting_t = MPI_Wtime() - sorting_s;


memset(recvCount,
0, bucketCount*sizeof(int));
MPI_Gather(&totalCount,
1, MPI_INT, recvCount, 1, MPI_INT,0, MPI_COMM_WORLD);
rdispls[0] = 0;
for (
i = 1;
i<bucketCount;
...
TRUE)){
fprintf(stdout,
"total time: %f, parallel: %f\n",

MPI_Wtime()

-total_s, bucketing_t+sorting_t);
fprintf(stdout,
"The data array has been sorted, from %f to %f\n", dSend[0], dSend[N-1]);
}
